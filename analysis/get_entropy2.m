function get_entropy2(s,datafile,epath,vjpath,varargin)

% Same as get_entropy, but top cdr3, VJ, V and J modified. Simply grabs the top cutoff specified number instead of attempting to group cdr3s first. More appropriate for measuring VJ information contribution.
...
switch s
	...
	case 1 % vj
	    ...
	    % sort by increasing counts
            [vjcounts,k]=sort(vjcounts,'descend');
            u_vj=u_vj(k);

            % apply cutoff
            u_vj=u_vj(1:cutoff);
            vjcounts=vjcounts(1:cutoff);	
	...
	case 2 % v
	    ...
	    % sort by increasing counts
            [vcounts,k]=sort(vcounts,'descend');
            u_v=u_v(k);

            % apply cutoff
            u_v=u_v(1:cutoff);
            vcounts=vcounts(1:cutoff);
	...
	case 3 % j
	    ...
	    for j=1:length(uS)
            ...
            % sort by increasing counts
            [jcounts,k]=sort(jcounts,'descend');
            u_j=u_j(k);

            % apply cutoff
            u_j=u_j(1:cutoff);
            jcounts=jcounts(1:cutoff);
