...
for (i in 3:n) {
		...
		if (flag == 0) {
				# plot
			plot(counts[,1]/sum(tcr[,i]), counts[,2], log="xy", col=i, xlab="clone frequency", ylab="# of clones", main=file , cex.lab=1.2, cex.axis=1.2, xlim=range(c(1e-7, 0.1)))
			flag = 1
		} else {
