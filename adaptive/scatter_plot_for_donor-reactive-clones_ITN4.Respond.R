...
### number of post-tx Stim donor reactive clones that are frequent in pre-tx stim sample
	...
	npreRxGT0 = nrow(topPostSti[topPostSti[,stimCol]> 0 | topPostSti[,preCol] > 0, ])
	
	npreRxFailed = nrow(topPostSti[(topPostSti[,stimCol] < freq1 |  topPostSti[,stimCol] <= topPostSti[, preCol] * fold ) & topPostSti[,stimCol]> 0 ,  ])
...
## panel 2, freq tracking for donor reactive clones from post-tx MLR
	...
	print(paste(npreRx, npreRxGT0, npreRxFailed), sep="\t")
