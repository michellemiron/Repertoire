...
compare <- function(tcr, freq1, freq2, fold=10) {
	...
	stimCol <- grep("stim_CD|stim2|stim3", colnames(tcr))
        ...
        postCols <- grep("pre|stim|anti|6mo", colnames(tcr), invert=T)
       

	postStimCol <- grep("posttx_antidonor", colnames(tcr))
	
#	postBaseLine <- grep("6moUn", colnames(tcr))
	postBaseLine <- grep("2y_CD", colnames(tcr))	
...
#         topStiRows = (tcr[,stimCol[1]] > tcr[, preCol] * fold & tcr[,stimCol[1]] >= 100 * freq1 )
        ...
        topStiRows = (tcr[,postStimCol] > tcr[, postBaseLine] * fold & tcr[,postStimCol] >= 100 * freq1 )
...
#        pretx3p = c(length(which(top3pSti[, preCol ] >= freq2 * 100)), length(which(top3pSti[, preCol] < freq2 * 100)))
	...
	pretx = c(length(which(topSti[, postBaseLine ] >= freq2 * 100)), length(which(topSti[, postBaseLine] < freq2 * 100)))

	cat(colnames(tcr)[postBaseLine],  pretx, "\n", sep="\t")
