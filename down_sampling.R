...
{
        r = rep(0,3)
	tcr = cbind(data$pretx_cd4, data$last_cd4)
	...
	cat("linear","linear-non-zero","log-non-zero","\n", sep="\t")
	cat(r, "\n", sep="\t")
...
cd4 = cbind(tcr$nucleotide, tcr$total, tcr$pretx_cd4, tcr$last_cd4)
cd8 = cbind(tcr$nucleotide, tcr$total, tcr$pretx_cd8, tcr$last_cd8)

downsample(cd4, nreads , noise)

downsample(cd8, nreads , noise)
